class php::extension::apcu {
  ...
  exec { '/bin/su - root -c "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/apcu-4.0.7 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"']
  }
