class php::extension::apcu {
  ...
  exec { 'phpize-5.4.33 apcu':
    command => '/phpfarm/inst/bin/phpize-5.4.33',
    cwd => '/tmp/apcu-4.0.7',
    require => Exec['tar xzf apcu-4.0.7.tgz']
  }

  exec { '/bin/su - root -mc "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"':
    timeout => 0,
    require => Exec['phpize-5.4.33 apcu']
  }

  exec { '/bin/su - root -mc "cd /tmp/apcu-4.0.7 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"']
  }
