class php::extension::zendopcache {
  ...
  exec { '/bin/su - root -mc "cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"':
    timeout => 0,
  ...
  exec { '/bin/su - root -mc "cd /tmp/zendopcache-7.0.3 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"']
  }
