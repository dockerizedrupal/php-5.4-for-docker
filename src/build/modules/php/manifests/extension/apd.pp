class php::extension::apd {
  ...
  exec { 'phpize-5.4.33 apd':
    command => '/phpfarm/inst/bin/phpize-5.4.33',
    cwd => '/tmp/pecl-apd-master',
    require => Exec['unzip pecl-apd-master.zip']
  }

  exec { '/bin/su - root -mc "cd /tmp/pecl-apd-master && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"':
    timeout => 0,
    require => Exec['phpize-5.4.33 apd']
  }

  exec { '/bin/su - root -mc "cd /tmp/pecl-apd-master && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/pecl-apd-master && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33"']
  }
