class php::extension::memcached {
  ...
  bash_exec { 'cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33 --enable-memcached-igbinary':
    timeout => 0,
  ...
  bash_exec { 'cd /tmp/memcached-2.2.0 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33 --enable-memcached-igbinary']
  }
