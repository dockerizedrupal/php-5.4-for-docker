class php::extension::memcached {
  ...
  exec { 'phpize-5.4.33 memcached':
    command => '/phpfarm/inst/bin/phpize-5.4.33',
    cwd => '/tmp/memcached-2.2.0',
    require => Exec['tar xzf memcached-2.2.0.tgz']
  }

  exec { '/bin/su - root -mc "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33 --enable-memcached-igbinary"':
    timeout => 0,
    require => Exec['phpize-5.4.33 memcached']
  }

  exec { '/bin/su - root -mc "cd /tmp/memcached-2.2.0 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.4.33 --enable-memcached-igbinary"']
  }
