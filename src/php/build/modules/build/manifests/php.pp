class build::php {
  ...
  bash_exec { 'mkdir -p /usr/local/src/phpfarm/inst/php-5.4.42/etc/fpm.d': }

  bash_exec { 'mkdir -p /usr/local/src/phpfarm/inst/php-5.4.42/etc/pool.d': }

  file { '/usr/local/src/phpfarm/inst/php-5.4.42/etc/pool.d/www.conf':
    ensure => present,
    source => 'puppet:///modules/build/usr/local/src/phpfarm/inst/php-5.4.42/etc/pool.d/www.conf',
    mode => 644,
    require => Bash_exec['mkdir -p /usr/local/src/phpfarm/inst/php-5.4.42/etc/pool.d']
  }
  ...
  file { '/tmp/php-5.4.42.tar.gz':
    ...
    source => 'puppet:///modules/build/tmp/php-5.4.42.tar.gz'
  }
  ...
  file { '/usr/local/src/phpfarm/src/custom/options-5.4.42.sh':
    ...
    source => 'puppet:///modules/build/usr/local/src/phpfarm/src/custom/options-5.4.42.sh',
    mode => 755,
  ...
  file { '/usr/local/src/phpfarm/inst/php-5.4.42/etc/php-fpm.conf':
    ...
    source => 'puppet:///modules/build/usr/local/src/phpfarm/inst/php-5.4.42/etc/php-fpm.conf',
    mode => 644,
    require => Bash_exec['/usr/local/src/phpfarm/src/main.sh 5.4.42']
  }
